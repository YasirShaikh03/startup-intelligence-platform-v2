...
//  Architecture: FastAPI backend → Claude API → Offline ML engine
// ══════════════════════════════════════════════════════════════════
...
// ── THEME MANAGEMENT ──────────────────────────────────────────────
function initTheme() {
  const saved = localStorage.getItem('sip_theme') || 'dark';
  document.documentElement.setAttribute('data-theme', saved);
  const ico = g('theme-ico');
  if (ico) ico.textContent = saved === 'light' ? '🌙' : '☀️';
}
function toggleTheme() {
  ...
  const ico = g('theme-ico');
  if (ico) ico.textContent = next === 'light' ? '🌙' : '☀️';
  // Re-apply chart colors when theme switches
  if (currentAnalysisScores) refreshChartColors();
}
function refreshChartColors() {
  // Charts auto-inherit CSS vars — just a trigger to re-render if needed
}

// ── BACKEND HEALTH CHECK ──────────────────────────────────────────
async function checkBackend() {
  const dot = g('backend-dot'), lbl = g('backend-lbl');
  if (dot) { dot.className = 'backend-dot checking'; }
  if (lbl) lbl.textContent = 'CHECKING';
  try {
    const r = await fetch(BACKEND_URL + '/health', { signal: AbortSignal.timeout(3000) });
    if (r.ok) {
      backendOnline = true;
      if (dot) dot.className = 'backend-dot online';
...
// ── BENCHMARK DATA (percentiles per category) ──
const BENCHMARKS = {
...
function exportJSON() {
  ...
  const report = {
    id: 'SIP-' + Date.now().toString(36).toUpperCase(),
    generated_at: new Date().toISOString(),
    business_name: d.name,
    biz_type: d.bizType,
    location: [d.area, d.city, d.state].filter(Boolean).join(', '),
    scores: {
      composite: sc2.compositeScore,
      survival: sc2.surv,
      profitability: sc2.prof,
      scalability: sc2.scal,
      local_dominance: sc2.ldom,
      ml_adjustment: sc2.mlAdj,
    },
    financials: {
      daily_revenue: d.dailyRev,
      monthly_revenue: d.monthlyRev,
      margin_pct: d.margin,
      growth_rate_pct: d.growthRate,
    },
    description: d.description,
    platform: 'Startup Intelligence Platform v2',
  };
  const b = new Blob([JSON.stringify(report, null, 2)], { type: 'application/json' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(b);
  a.download = (d.name || 'analysis').replace(/\s+/g,'_') + '_SIP_report.json';
  a.click();
}
...
function renderAI(ai, isOffline, sourceLbl) {
  const aiScore = ai.gemini_score || ai.ai_score || 0;
  const src     = sourceLbl || (isOffline ? 'OFFLINE ENGINE' : 'CLAUDE SONNET');
  const srcCls  = src.includes('FASTAPI') ? 'api' : src.includes('CLAUDE') ? 'claude' : 'offline';
  return `
    <div class="card ai-card fade-in" style="margin-bottom:.85rem">
      <div class="sec-title">🤖 AI Deep Analysis <span class="ai-badge">${src}</span></div>
      ...
      ${aiScore ? `<div class="ai-score-box">
        ...
        <div class="ai-score-info">
          ...
          <div class="ai-score-desc">Independent AI assessment — ${aiScore>=70?'strong fundamentals detected':aiScore>=50?'moderate potential with addressable gaps':'significant challenges identified'}.</div>
        </div>
      </div>` : ''}
...
function renderResult(d, sc2, isS) {
  ...
  g('rv').innerHTML = `<div class="fade-in">
    ...
    <div class="card" style="text-align:center;margin-bottom:.85rem">
      ...
      <div class="exp-row-v2">
        <button class="btn-exp-v2 json" onclick="exportJSON()">⬇ JSON Report</button>
        <button class="btn-exp-v2 pdf" onclick="exportPDF()">📄 PDF Report</button>
        <button class="btn-exp-v2 share" onclick="shareReport()">🔗 Share Report</button>
        <button class="btn-exp-v2" onclick="window.print()" style="border-color:rgba(167,139,250,.35);color:var(--pu)">🖨 Print</button>
      </div>
    </div>
...
/* ── Claude API call (replaces Gemini backend) ── */

/* ══════════════════════════════════════════════════════════
   OFFLINE AI ENGINE — 100% local, no internet needed
   Generates personalised analysis from form data + ML scores
══════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════════
   ...
   Priority: FastAPI Backend → Claude API → Offline ML Engine
══════════════════════════════════════════════════════════════════ */
async function runAnalysis() {
  ...
  let scores, aiInsightsFromBackend = null;
  if (backendOnline) {
    try {
      const payload = buildBackendPayload(d, isS);
      ...
      if (res.ok) {
        ...
        // Map backend score format to frontend format
        scores = mapBackendScores(json.scores, isS);
        aiInsightsFromBackend = json.insights;
        aiInsightsFromBackend._source = 'backend';
      }
    } catch(_) { /* fall through to local scoring */ }
  }

  // ── Fallback: local JS scoring engine ─────────────────────────
  if (!scores) {
    scores = isS ? computeStreet(d) : computeStartup(d);
  }

  await new Promise(r => setTimeout(r, 1500)); // minimum UX delay
  stopLoader();
  g('lv').classList.add('hidden');

  currentAnalysisData = d;
  currentAnalysisScores = scores;
  currentAnalysisIsStreet = isS;
  chatMessages = [];
  ...
  // ── AI Insights: backend result OR Claude API OR offline ───────
  const ph = g('ai-ph');
  if (!ph) return;

  const scoresSummary = { compositeScore: scores.compositeScore, surv: scores.surv, prof: scores.prof, scal: scores.scal, ldom: scores.ldom };

  if (aiInsightsFromBackend) {
    // Backend returned full analysis
    ph.innerHTML = renderAI(aiInsightsFromBackend, false, 'FASTAPI ENGINE');
    return;
  }

  if (!d.description || d.description.trim().length <= 10) {
    ph.innerHTML = `<div class="card" style="margin-bottom:.85rem;border-style:dashed;opacity:.55"><div class="sec-title">🤖 AI Deep Analysis</div><p style="color:var(--mu);font-size:.83rem;line-height:1.65">Fill in the <strong style="color:var(--tx)">Business Description</strong> field above to unlock personalised AI analysis.</p></div>`;
    return;
  }

  // Show loading state
  ph.innerHTML = `<div class="card ai-card" style="margin-bottom:.85rem;border-color:rgba(167,139,250,.28)"><div class="ai-spin-wrap"><div class="ai-spinner"></div><div><div class="ai-ld-title">🤖 GENERATING AI ANALYSIS...</div><div class="ai-ld-sub">Trying Claude API → offline fallback. Always works.</div></div></div></div>`;

  const offlineResult = offlineAnalysis(d, scoresSummary);

  if (!navigator.onLine) {
    ph.innerHTML = renderAI(offlineResult, true, 'OFFLINE ENGINE');
    return;
  }

  // Try Claude API
  try {
    const prompt = buildClaudePrompt(d, scores, isS);
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 2000, messages: [{ role: 'user', content: prompt }] }),
    });
    if (!res.ok) throw new Error('claude-api');
    ...
    ph.innerHTML = renderAI(ai || offlineResult, !ai, ai ? 'CLAUDE SONNET' : 'OFFLINE ENGINE');
  } catch(_) {
    ph.innerHTML = renderAI(offlineResult, true, 'OFFLINE ENGINE');
  }
}

/* Build payload for FastAPI backend */
function buildBackendPayload(d, isS) {
  return {
    name: d.name || 'Unnamed Business',
    biz_type: d.bizType || '',
    industry: d.industry || '',
    city: d.city || '',
    state: d.state || '',
    ...
    daily_cust: d.dailyCust || 0,
    avg_price: d.avgPrice || 0,
    hygiene: d.hygiene || '',
    repeat_rate: d.repeatRate || 0,
    taste_rating: d.tasteRating || 0,
    daily_waste: d.dailyWaste || 0,
    rm_var: d.rmVar || '',
    footfall: d.footfall || '',
    landmarks: d.landmarks || '',
    dist_sta: d.distSta || '',
    dist_col: d.distCol || '',
    area_comp: d.areaComp || '',
    crowd_type: d.crowdType || '',
    fexp: d.fexp || '',
    tsize: d.tsize || '',
    rev: d.rev || '',
    pstage: d.pstage || '',
    ...
    famt: d.famt || '',
    msize: d.msize || '',
    daily_rev: d.dailyRev || 0,
    monthly_rev: d.monthlyRev || 0,
    margin: d.margin || 0,
    growth_rate: d.growthRate || 0,
    cost_unit: d.costUnit || 0,
    sell_price: d.sellPrice || 0,
...
/* Map backend snake_case scores to frontend camelCase format */
function mapBackendScores(bs, isS) {
  return {
    compositeScore: bs.composite_score,
    surv: bs.surv, prof: bs.prof, scal: bs.scal, ldom: bs.ldom,
    tracS: bs.trac_s || bs.surv,
    mktS:  bs.mkt_s  || bs.scal,
    teamS: bs.team_s || 50,
    funS:  bs.fun_s  || 50,
    moatS: bs.moat_s || bs.ldom,
    locS:  bs.loc_s  || bs.ldom,
    prodS: bs.prod_s || bs.prof,
    finS:  bs.fin_s  || bs.prof,
    opsS:  bs.ops_s  || 50,
    grwS:  bs.grw_s  || bs.scal,
    mlAdj: bs.ml_adj || 0,
    fm:    bs.fm     || 0.68,
    fd:    bs.fd     || 60,
    hy:    bs.hy     || 50,
...
/* Build Claude prompt */
function buildClaudePrompt(d, scores, isS) {
  return `You are an elite startup and street business consultant. Return ONLY valid JSON.

Business: ${d.name} | Type: ${d.bizType||''} | Mode: ${isS?'Street Food':'Startup'} | Location: ${loc(d)}
Score: ${scores.compositeScore}/100 | Survival: ${scores.surv} | Profit: ${scores.prof} | Scale: ${scores.scal} | Local: ${scores.ldom}
${d.description ? `Description: "${d.description}"` : ''}
${d.margin ? `Margin: ${d.margin}%` : ''} ${d.growthRate ? `Growth: ${d.growthRate}%/mo` : ''}

Return exactly this JSON (no markdown):
{
  "summary": "3-4 sentence executive summary mentioning the business name and specific data",
  "strengths": ["strength 1 with specific data","strength 2","strength 3","strength 4","strength 5","strength 6"],
  "risks": ["risk 1 specific","risk 2","risk 3","risk 4","risk 5","risk 6"],
  "opportunities": ["opp 1","opp 2","opp 3","opp 4"],
  "actions": ["action 1 this week","action 2","action 3","action 4","action 5"],
  "scaling": "2-3 paragraph scaling strategy with specific milestones",
  "verdict": "2-3 sentence investment/viability verdict",
  "hidden_insight": "1-2 sentence non-obvious insight most consultants would miss",
  "gemini_score": ${Math.round(scores.compositeScore * 0.9 + 5)}
}`;
}
...
/* ═══════════════════════════════════════════════════════════════
   ...
   Shows how the user's scores compare to category averages
═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE 2 — ADVANCED REVENUE FORECAST (18-month interactive)
═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE 4 — PDF REPORT EXPORT (print-to-PDF with styles)
═══════════════════════════════════════════════════════════════ */
function exportPDF() {
  const d   = currentAnalysisData;
  const sc2 = currentAnalysisScores;
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE: WHAT-IF SCENARIO ENGINE
═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE: FUNDING READINESS METER
═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE: GST & COMPLIANCE CHECKLIST
═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE: INVESTOR PITCH DECK GENERATOR (Claude AI)
═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE: AI BUSINESS PLAN WRITER (Claude AI)
═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   FEATURE: COMPARISON ENGINE  —  min 2, max 5 from history
═══════════════════════════════════════════════════════════════ */
...
/* Called after result renders — shows ALL history entries as pickable cards */
...
function toggleCmp(idx) {
  ...
  // Gray-out remaining cards once at 5
